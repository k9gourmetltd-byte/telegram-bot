...
// ============ DATABASE (JSON files - persistent) ============
...
// ============ ALPHA VANTAGE (Stocks - Official API) ============
...
// ============ CRYPTO SIGNAL — True Multi-TF, No Hidden Fallback ============
async function genCryptoSignal(symbol) {
  ...
  // TRUE multi-timeframe — each from its own kline data
  ...
  // NO HIDDEN FALLBACK — must meet consensus or return null
  let dir = null;
  if (lv >= 3) dir = 'LONG';
  else if (sv >= 3) dir = 'SHORT';
  else if (lv === 2 && sv <= 1 && depth.imbalance > 10) dir = 'LONG';
  else if (sv === 2 && lv <= 1 && depth.imbalance < -10) dir = 'SHORT';
  
  if (!dir) return null; // NO consensus — no signal
...
// ============ FULL SIGNAL FORMAT (Nothing Stripped) ============
...
bot.on('message', async msg => { const uid = msg.from?.id; if (!isAuth(uid)) return; const t = msg.text || '', cid = msg.chat.id; if (t.startsWith('/')) return;
  ...
  else if (t.includes('Dashboard')) { bot.sendMessage(cid, '📊 *Dashboard*\n\nTap below:', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🚀 Open Dashboard', web_app: { url: (WEBAPP_URL || 'http://localhost:' + PORT) + '/dashboard/' + uid } }]] } }); } });

// TP/SL BUTTONS — Write to persistent database
bot.on('callback_query', async q => { const uid = q.from.id; if (!isAuth(uid)) return bot.answerCallbackQuery(q.id); const cid = q.message.chat.id, d = q.data, state = loadUser(uid);
  if (d.startsWith('exec_')) { const [, id, sz] = d.split('_'); const sig = state.signals.find(x => x.id === id); if (sig) { state.positions.unshift({ id: crypto.randomBytes(4).toString('hex'), symbol: sig.symbol, direction: sig.direction, entry: sig.entry, size: sz, status: 'open', openedAt: new Date().toISOString(), pnl: 0, pnlPercent: 0 }); state.stats.t++; saveUser(uid, state); bot.answerCallbackQuery(q.id, { text: '✅ Trade opened!' }); bot.sendMessage(cid, '✅ *Trade Opened*\n' + sig.direction + ' ' + sig.symbol + ' @ $' + sig.entry + '\nSize: ' + sz + '%\n📋 Track in My Positions', { parse_mode: 'Markdown' }); } else { bot.answerCallbackQuery(q.id, { text: 'Expired' }); } }
  else if (d.startsWith('tp_')) { const id = d.replace('tp_', ''); const pos = state.positions.find(x => x.signalId === id && x.status === 'open') || state.positions[0]; if (pos) { const pnl = 1.5; pos.status = 'closed'; pos.closedAt = new Date().toISOString(); pos.closeReason = 'TP1'; pos.pnlPercent = pnl; state.stats.w++; state.stats.s++; state.stats.p += pnl; state.history.unshift(pos); state.positions = state.positions.filter(x => x.id !== pos.id); saveUser(uid, state); GS.totalWins++; GS.totalSignals++; saveGS(GS); bot.answerCallbackQuery(q.id, { text: '🎯 TP1 +1.5%' }); bot.sendMessage(cid, '🎯 *TP1 HIT!* +1.5%\nStreak: ' + state.stats.s + ' 🔥\n📊 Global stats updated', { parse_mode: 'Markdown' }); } else { bot.answerCallbackQuery(q.id, { text: 'No position found' }); } }
  else if (d.startsWith('sl_')) { const id = d.replace('sl_', ''); const pos = state.positions.find(x => x.signalId === id && x.status === 'open') || state.positions[0]; if (pos) { const loss = -1.2; pos.status = 'closed'; pos.closedAt = new Date().toISOString(); pos.closeReason = 'SL'; pos.pnlPercent = loss; state.stats.l++; state.stats.s = 0; state.stats.p += loss; state.history.unshift(pos); state.positions = state.positions.filter(x => x.id !== pos.id); saveUser(uid, state); GS.totalLosses++; GS.totalSignals++; saveGS(GS); bot.answerCallbackQuery(q.id, { text: '🛑 SL -1.2%' }); bot.sendMessage(cid, '🛑 *SL Hit* -1.2%\nStreak reset\n📊 Global stats updated', { parse_mode: 'Markdown' }); } else { bot.answerCallbackQuery(q.id, { text: 'No position found' }); } }
