...
async function getStockPrice(s) { try { const r = await fetchWithRetry("https://api.twelvedata.com/price?symbol=" + s + "async function getStockPrice(s) { let p = await getStockPriceAV(s); if (p) return p;apikey=demo", {}, 2); const price = parseFloat(r.data.price); if (price) return price; } catch(e) {} let p = await getStockPriceAV(s); if (p) return p; try { const r = await fetchWithRetry('https://query1.finance.yahoo.com/v8/finance/chart/' + s, { headers: { 'User-Agent': UA } }, 2); return r.data.chart.result[0].meta.regularMarketPrice; } catch (e) { return null; } }
async function getStockData(s) { try { const r = await fetchWithRetry("https://api.twelvedata.com/time_series?symbol=" + s + "async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d;interval=5minasync function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d;outputsize=50async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d;apikey=demo", {}, 2); const values = r.data.values; if (values async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d;async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d; values.length > 10) { return { closes: values.map(v => parseFloat(v.close)).reverse(), highs: values.map(v => parseFloat(v.high)).reverse(), lows: values.map(v => parseFloat(v.low)).reverse() }; } } catch(e) {} let d = await getStockDataAV(s); if (d async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d;async function getStockData(s) { let d = await getStockDataAV(s); if (d && d.closes.length > 10) return d; d.closes.length > 10) return d; try { const r = await fetchWithRetry('https://query1.finance.yahoo.com/v8/finance/chart/' + s, { headers: { 'User-Agent': UA } }, 2); const q = r.data.chart.result[0].indicators.quote[0]; return { closes: (q.close || []).filter(c => c !== null), highs: (q.high || []).filter(h => h !== null), lows: (q.low || []).filter(l => l !== null) }; } catch (e) { return { closes: [], highs: [], lows: [] }; } }
...
async function genStockSignal(symbol) {
  ...
  return { id: crypto.randomBytes(4).toString('hex'), type: 'stock', symbol, direction: dir, price: price.toFixed(2), entry: e.toFixed(2), tp1, tp2, tp3, sl, score: sc, rsi1h: rsi.toFixed(1), macd, sr, depth: { imbalance: 'N/A', spread: 'N/A', bestBid: 'N/A', bestAsk: 'N/A', bidWalls: [], askWalls: [] }, rr1: (Math.abs(tp1 - e) / Math.abs(e - sl)).toFixed(1), funding: { rate: 'N/A', sentiment: 'N/A' }, lsRatio: { long: 'N/A', sentiment: 'N/A' }, oi: 'N/A', vs: '1.0', reasons: [(lv >= 2 ? lv : sv) + '/3 timeframes aligned'], warnings: [], st, ema, tf5m, tf15m, tf1h, tf4h: { label:'4H',bias:'N/A',emoji:'⚪',rsi:'--',macd:'--',st:'--'}, longVotes: lv, shortVotes: sv, timestamp: new Date().toISOString() };
}
...
server.listen(PORT, '0.0.0.0', () => { console.log('K9 SignalBot v7 running on port ' + PORT); broadcast(); setInterval(broadcast, 15 * 60 * 1000); setInterval(updatePrices, 5000); updatePrices(); });
