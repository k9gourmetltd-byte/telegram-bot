...
function getLeverageInfo(score, warnings, assetType, rr) {
  ...
  return { maxLev, recLev, riskPct, posSize, leverageRatio, rrNum };
}
...
function loadUser(uid) { try { if (fs.existsSync(userFile(uid))) return JSON.parse(fs.readFileSync(userFile(uid), 'utf8')); } catch (e) {} return { signals: [], positions: [], history: [], stats: { t: 0, w: 0, l: 0, p: 0, s: 0 }, strictMode: true }; }
...
bot.onText(/\/start/, msg => { const uid = msg.from?.id; if (!isAuth(uid)) return bot.sendMessage(msg.chat.id, '⛔ Unauthorized'); bot.sendMessage(msg.chat.id, '🤖 *K9 SignalBot v11*\n\n🎯 Strict 3/4 TF Consensus\n⚠️ /force — Relaxed consensus (trade at own risk)\n📊 Leverage + Position Size on every signal\n₿ Crypto • 📈 Stocks • 🥇 Metals\n🔁 Rate-limited APIs • 🕐 Market hours\n📡 Free: @k9signalalerts\n\n👇 Menu:', { parse_mode: 'Markdown', ...CM }); });

bot.onText(/\/signal(?:\s+(.+))?/, async (msg, match) => { const uid = msg.from?.id; if (!isAuth(uid)) return; const input = match?.[1]; if (!input) return bot.sendMessage(msg.chat.id, '/signal btc | eth | sol | aapl | tsla | gold | silver', { parse_mode: 'Markdown' }); let sym = input.toUpperCase(); if (METAL_ALIASES[input.toLowerCase()]) { const mSym = METAL_ALIASES[input.toLowerCase()]; const meta = METALS.find(m => m.symbol === mSym); const s = await genSignal(mSym, 'metal', meta); if (!s) return bot.sendMessage(msg.chat.id, '⚪ *No Clear Signal*\n\nStrict consensus not met. Reply /force_' + input.toLowerCase() + ' for relaxed signal (trade at own risk).', { parse_mode: 'Markdown' }); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); return; } if (TOP_STOCKS.includes(sym)) { const s = await genSignal(sym, 'stock'); if (!s) return bot.sendMessage(msg.chat.id, '⚪ *No Clear Signal*\n\nStrict consensus not met. Reply /force_' + sym.toLowerCase() + ' for relaxed signal.', { parse_mode: 'Markdown' }); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); return; } if (!sym.endsWith('USDT')) sym = COINS[input.toLowerCase()] || sym + 'USDT'; const s = await genSignal(sym, 'crypto'); if (!s) return bot.sendMessage(msg.chat.id, '⚪ *No Clear Signal*\n\nStrict 3/4 consensus not met.\n\n⚠️ Reply /force ' + input.toLowerCase() + ' for relaxed signal (lower conviction, trade at own risk).', { parse_mode: 'Markdown' }); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '✅ Trade 25%', callback_data: 'exec_' + s.id + '_25' }], [{ text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }, { text: '🛑 SL Hit', callback_data: 'sl_' + s.id }]] } }); });

bot.onText(/\/force(?:\s+(.+))?/, async (msg, match) => { const uid = msg.from?.id; if (!isAuth(uid)) return; const input = match?.[1]; if (!input) return bot.sendMessage(msg.chat.id, '/force btc | eth | sol | aapl | gold', { parse_mode: 'Markdown' }); let sym = input.toUpperCase(); if (METAL_ALIASES[input.toLowerCase()]) { sym = METAL_ALIASES[input.toLowerCase()]; const meta = METALS.find(m => m.symbol === sym); const s = await genSignal(sym, 'metal', meta, true); if (!s) return bot.sendMessage(msg.chat.id, '⚪ Still no signal even with relaxed consensus.'); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); return; } if (TOP_STOCKS.includes(sym)) { const s = await genSignal(sym, 'stock', null, true); if (!s) return bot.sendMessage(msg.chat.id, '⚪ Still no signal.'); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); return; } if (!sym.endsWith('USDT')) sym = COINS[input.toLowerCase()] || sym + 'USDT'; const s = await genSignal(sym, 'crypto', null, true); if (!s) return bot.sendMessage(msg.chat.id, '⚪ Still no signal even with relaxed consensus.'); bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '✅ Trade 25%', callback_data: 'exec_' + s.id + '_25' }], [{ text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }, { text: '🛑 SL Hit', callback_data: 'sl_' + s.id }]] } }); });

bot.onText(/\/force_(.+)/, async (msg, match) => { const uid = msg.from?.id; if (!isAuth(uid)) return; const input = match[1]; let sym = input.toUpperCase(); if (METAL_ALIASES[input.toLowerCase()]) { sym = METAL_ALIASES[input.toLowerCase()]; const meta = METALS.find(m => m.symbol === sym); const s = await genSignal(sym, 'metal', meta, true); if (s) bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); return; } if (TOP_STOCKS.includes(sym)) { const s = await genSignal(sym, 'stock', null, true); if (s) bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown' }); return; } if (!sym.endsWith('USDT')) sym = COINS[input.toLowerCase()] || sym + 'USDT'; const s = await genSignal(sym, 'crypto', null, true); if (s) bot.sendMessage(msg.chat.id, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); });
...
bot.on('message', async msg => { const uid = msg.from?.id; if (!isAuth(uid)) return; const t = msg.text || '', cid = msg.chat.id; if (t.startsWith('/')) return;
  if (t.includes('GOLD') || t.includes('SILVER') || t.includes('PLATINUM') || t.includes('PALLADIUM') || t.includes('COPPER')) { const mm = { GOLD: 'XAUUSD', SILVER: 'XAGUSD', PLATINUM: 'XPTUSD', PALLADIUM: 'XPDUSD', COPPER: 'XCUUSD' }; const mSym = Object.keys(mm).find(k => t.toUpperCase().includes(k)); if (mSym && mm[mSym]) { const meta = METALS.find(m => m.symbol === mm[mSym]); const s = await genSignal(mm[mSym], 'metal', meta); if (!s) return bot.sendMessage(cid, '⚪ *No Clear Signal*\n\nReply /force_' + mSym.toLowerCase() + ' for relaxed signal.', { parse_mode: 'Markdown' }); bot.sendMessage(cid, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); } }
  else if (TOP_STOCKS.some(s => t.includes(s) && t.includes('📈'))) { const sym = TOP_STOCKS.find(s => t.includes(s)); if (sym) { const s = await genSignal(sym, 'stock'); if (!s) return bot.sendMessage(cid, '⚪ *No Clear Signal*\n\nReply /force_' + sym.toLowerCase() + ' for relaxed signal.', { parse_mode: 'Markdown' }); bot.sendMessage(cid, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }]] } }); } }
  else if (t.includes('BTC Signal') || t.includes('ETH Signal') || t.includes('SOL Signal')) { let sym = 'BTCUSDT'; if (t.includes('ETH')) sym = 'ETHUSDT'; else if (t.includes('SOL')) sym = 'SOLUSDT'; const s = await genSignal(sym, 'crypto'); if (!s) return bot.sendMessage(cid, '⚪ *No Clear Signal*\n\nStrict 3/4 consensus not met.\n⚠️ Reply /force ' + sym.toLowerCase().replace('usdt','') + ' for relaxed signal.', { parse_mode: 'Markdown' }); bot.sendMessage(cid, fullSignal(s), { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '✅ Trade 10%', callback_data: 'exec_' + s.id + '_10' }, { text: '✅ Trade 25%', callback_data: 'exec_' + s.id + '_25' }], [{ text: '🎯 TP1 Hit', callback_data: 'tp_' + s.id }, { text: '🛑 SL Hit', callback_data: 'sl_' + s.id }]] } }); }
...
server.listen(PORT, '0.0.0.0', () => { console.log('K9 SignalBot v11 running on port ' + PORT); broadcast(); setInterval(broadcast, 15 * 60 * 1000); setInterval(updatePrices, 5000); updatePrices(); });
