...
// BINANCE
async function getPrice(s) { try { const r = await axios.get('https://api.binance.com/api/v3/ticker/price?symbol=' + s, { timeout: 3000 }); return parseFloat(r.data.price); } catch (e) { return null; } }
async function get24hr(s) { try { const r = await axios.get('https://api.binance.com/api/v3/ticker/24hr?symbol=' + s, { timeout: 3000 }); return { high: parseFloat(r.data.highPrice), low: parseFloat(r.data.lowPrice), change: parseFloat(r.data.priceChangePercent), volume: parseFloat(r.data.quoteVolume) }; } catch (e) { return null; } }
async function getKlines(s, i, l) { try { const r = await axios.get('https://api.binance.com/api/v3/klines?symbol=' + s + '&interval=' + i + '&limit=' + l, { timeout: 3000 }); return r.data.map(k => ({ close: parseFloat(k[4]), high: parseFloat(k[2]), low: parseFloat(k[3]), volume: parseFloat(k[5]) })); } catch (e) { return []; } }
async function getDepth(s) { try { const r = await axios.get('https://api.binance.com/api/v3/depth?symbol=' + s + '&limit=100', { timeout: 3000 }); const bids = r.data.bids.map(b => ({ p: parseFloat(b[0]), q: parseFloat(b[1]) })), asks = r.data.asks.map(a => ({ p: parseFloat(a[0]), q: parseFloat(a[1]) })); const bv = bids.reduce((x, b) => x + b.q * b.p, 0), av = asks.reduce((x, a) => x + a.q * a.p, 0); const bw = findWalls(bids), aw = findWalls(asks); return { imbalance: +(((bv - av) / (bv + av)) * 100).toFixed(1), spread: +((asks[0].p - bids[0].p).toFixed(2)), spreadPct: +(((asks[0].p - bids[0].p) / bids[0].p) * 100).toFixed(3), bestBid: bids[0].p, bestAsk: asks[0].p, bidWalls: bw.slice(0, 3).map(w => ({ price: w.price.toFixed(2), qty: w.totalQty.toFixed(2), orders: w.count })), askWalls: aw.slice(0, 3).map(w => ({ price: w.price.toFixed(2), qty: w.totalQty.toFixed(2), orders: w.count })) }; } catch (e) { return null; } }
...
async function getFunding(s) { try { const r = await axios.get('https://fapi.binance.com/fapi/v1/fundingRate?symbol=' + s + '&limit=1', { timeout: 3000 }); const rate = parseFloat(r.data[0]?.fundingRate || 0) * 100; return { rate: rate.toFixed(4), sentiment: rate > 0.05 ? 'BEARISH' : rate < -0.05 ? 'BULLISH' : 'NEUTRAL' }; } catch (e) { return { rate: '0', sentiment: 'NEUTRAL' }; } }
async function getLSRatio(s) { try { const r = await axios.get('https://fapi.binance.com/fapi/v1/globalLongShortAccountRatio?symbol=' + s + '&period=5m&limit=1', { timeout: 3000 }); const ratio = parseFloat(r.data[0]?.longShortRatio || 1); return { long: ratio.toFixed(1), sentiment: ratio > 1.5 ? 'BEARISH (crowded longs)' : ratio < 0.7 ? 'BULLISH (crowded shorts)' : 'NEUTRAL' }; } catch (e) { return { long: '1.0', sentiment: 'NEUTRAL' }; } }
async function getOpenInterest(s) { try { const r = await axios.get('https://fapi.binance.com/fapi/v1/openInterest?symbol=' + s, { timeout: 3000 }); return (parseFloat(r.data.openInterest) / 1e9).toFixed(1) + 'B'; } catch (e) { return 'N/A'; } }

// YAHOO STOCKS (FIXED)
async function getStockPrice(s) { try { const r = await axios.get('https://query1.finance.yahoo.com/v8/finance/chart/' + s, { params: { interval: '5m', range: '1d' }, timeout: 8000, headers: { 'User-Agent': UA } }); return r.data.chart.result[0].meta.regularMarketPrice; } catch (e) { return null; } }
async function getStockData(s) { try { const r = await axios.get('https://query1.finance.yahoo.com/v8/finance/chart/' + s, { params: { interval: '5m', range: '1d' }, timeout: 8000, headers: { 'User-Agent': UA } }); const q = r.data.chart.result[0].indicators.quote[0]; return { closes: (q.close || []).filter(c => c !== null), highs: (q.high || []).filter(h => h !== null), lows: (q.low || []).filter(l => l !== null) }; } catch (e) { return { closes: [], highs: [], lows: [] }; } }
...
async function getPolymarket() { try { const r = await axios.get('https://gamma-api.polymarket.com/markets?limit=6&order=volume&ascending=false', { timeout: 5000 }); if (Array.isArray(r.data)) return r.data.slice(0, 6).map(m => ({ title: m.title || m.question || '', volume: (parseFloat(m.volumeNum || m.volume || 0) / 1e6).toFixed(1) + 'M', prices: [m.outcomePrices ? JSON.parse(m.outcomePrices)[0] : '0'].map(p => (parseFloat(p) * 100).toFixed(0) + '%') })); } catch (e) { return null; } }
...
async function genCryptoSignal(symbol) {
  ...
  if (funding.sentiment === 'BEARISH' && dir === 'LONG') warnings.push('Funding opposes LONG');
  if (funding.sentiment === 'BULLISH' && dir === 'SHORT') warnings.push('Funding opposes SHORT');
  if (macd1h.trend === 'BEARISH' && dir === 'LONG') warnings.push('MACD opposes LONG');
  if (macd1h.trend === 'BULLISH' && dir === 'SHORT') warnings.push('MACD opposes SHORT');
  if (ema.slowCross !== (dir === 'LONG' ? 'BULLISH' : 'BEARISH')) warnings.push('Golden/Death cross opposes');
  ...
  const vol = symbol.includes('BTC') ? 0.008 : symbol.includes('ETH') ? 0.012 : 0.020;
...
async function genStockSignal(symbol) {
  ...
  if (!price || !data.closes || data.closes.length < 20) return null;
  ...
  const lv = votes.filter(v => v === 'long').length, sv = votes.filter(v => v === 'short').length;
  let dir = null;
  if (lv >= 2) dir = 'LONG'; else if (sv >= 2) dir = 'SHORT'; else return null;
...
function fullSignal(s) {
  ...
  return label + ' • ' + emoji + ' *' + s.direction + ' ' + s.symbol + '*\n\n' +
    ...
    extra + warnText + '\n\n💡 *Signals:* ' + s.reasons.slice(0, 6).join(' • ') + '\n\n⚡ *Score:* ' + stars + ' ' + s.score + '/95 | Consensus: ' + (s.longVotes || 0) + 'L/' + (s.shortVotes || 0) + 'S';
}
...
bot.on('message', async msg => { const uid = msg.from?.id; if (!isAuth(uid)) return; const t = msg.text || '', cid = msg.chat.id; if (t.startsWith('/')) return;
  ...
  else if (t.includes('Dashboard')) { bot.sendMessage(cid, '📊 *Dashboard*\n\nTap below:', { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: '🚀 Open Dashboard', web_app: { url: WEBAPP_URL + '/dashboard/' + uid } }]] } }); } });
...
app.get('/dashboard/:uid', (req, res) => { if (!isAuth(parseInt(req.params.uid))) return res.status(403).send('Unauthorized'); res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>K9 SignalBot</title><script src="https://cdn.socket.io/4.7.2/socket.io.min.js"></script><style>:root{--bg:#060b10;--bg2:#0d1520;--text:#e8edf3;--text2:#8899b4;--accent:#F0B90B;--green:#00d4aa;--red:#ff4757}*{box-sizing:border-box;margin:0;padding:0}body{background:var(--bg);color:var(--text);font-family:Inter,sans-serif}.app{max-width:1200px;margin:0 auto;padding:20px}.header{background:var(--bg2);border-radius:12px;padding:20px;margin-bottom:20px;display:flex;justify-content:space-between}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}.panel{background:var(--bg2);border-radius:12px;padding:20px}.panel-title{font-size:12px;color:var(--text2);text-transform:uppercase;margin-bottom:12px}.signal-row,.price-row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid rgba(255,255,255,0.06);font-size:13px}.stat-big{font-size:28px;font-weight:700}.green{color:var(--green)}.red{color:var(--red)}.accent{color:var(--accent)}</style></head><body><div class="app"><div class="header"><h1>🤖 K9 SignalBot</h1><span class="green">🟢 Live</span></div><div class="grid"><div class="panel"><div class="panel-title">🧠 Signals</div><div id="signals">Loading...</div></div><div class="panel"><div class="panel-title">📊 Stats</div><div id="stats">Loading...</div></div><div class="panel"><div class="panel-title">🌐 Global</div><div id="global">Loading...</div></div><div class="panel"><div class="panel-title">💰 Prices</div><div id="prices">Loading...</div></div></div></div><script>const s=io();s.on("stateUpdate-' + req.params.uid + '",st=>{if(st.signals)document.getElementById("signals").innerHTML=st.signals.slice(0,6).map(x=>\'<div class="signal-row"><span>\'+(x.direction=="LONG"?"🟢":"🔴")+" "+x.symbol+\'</span><span>\'+x.score+\'/95</span></div>\').join("");if(st.stats){const t=st.stats.w+st.stats.l;document.getElementById("stats").innerHTML="Total:"+st.stats.t+" | Wins:"+st.stats.w+" | WR:"+(t>0?((st.stats.w/t)*100).toFixed(1):"0")+"%";}});fetch("/api/globalstats").then(r=>r.json()).then(g=>{const t=g.totalWins+g.totalLosses;document.getElementById("global").innerHTML=\'<div class="stat-big accent">\'+(g.totalSignals||0)+\'</div><div>Signals | WR: \'+(t>0?((g.totalWins/t)*100).toFixed(1):"--")+\'%</div>\';});s.on("priceUpdate",p=>{let h="";for(const[k,v]of Object.entries(p||{}))h+=\'<div class="price-row"><span>\'+k+\'</span><span>\'+(v.price?.toFixed(2)||"--")+\'</span></div>\';document.getElementById("prices").innerHTML=h;})</script></body></html>'); });
...
server.listen(PORT, '0.0.0.0', () => { console.log('K9 SignalBot running on port ' + PORT); broadcast(); setInterval(broadcast, 15 * 60 * 1000); setInterval(updatePrices, 5000); updatePrices(); });
