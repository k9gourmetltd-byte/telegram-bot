...
async function broadcast() { const sigs = []; for (const sym of PRIORITY_PAIRS) { try { const s = await genSignal(sym, "crypto"); if (s) sigs.push(s); } catch (e) {} }
  if (sigs.length === 0) { for (const sym of PRIORITY_PAIRS) { try { const r = await getPrice(sym); const k = await getKlines(sym, "1h", 30); if (r async function broadcast() { const sigs = []; for (const sym of PRIORITY_PAIRS) { try { const s = await genSignal(sym, 'crypto'); if (s) sigs.push(s); } catch (e) {} }async function broadcast() { const sigs = []; for (const sym of PRIORITY_PAIRS) { try { const s = await genSignal(sym, 'crypto'); if (s) sigs.push(s); } catch (e) {} } k.length > 10) { const rsi = calcRSI(k.map(x => x.close)); const dir = rsi > 55 ? "LONG" : rsi < 45 ? "SHORT" : "LONG"; sigs.push({ symbol: sym, direction: dir, price: r.toFixed(2), rsi1h: rsi.toFixed(1), longVotes: 0, shortVotes: 0 }); } } catch(e) {} } } if (sigs.length > 0 && SIGNAL_CHANNEL !== 'none') { const t = GS.totalWins + GS.totalLosses; try { await bot.sendMessage(SIGNAL_CHANNEL, '🤖 *K9 • ' + new Date().toLocaleString() + '*\n\n' + sigs.map(s => freeSignal(s)).join('\n\n') + '\n\n📊 ' + GS.totalSignals + ' signals | ' + (t > 0 ? ((GS.totalWins / t) * 100).toFixed(1) : '--') + '% WR\n💎 @K9sigbot — $249/mo\n\n🔐 K9 SignalBot • Verified Consensus Engine', { parse_mode: 'Markdown' }); } catch (e) {} } }
